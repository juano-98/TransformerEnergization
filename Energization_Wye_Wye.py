...
# 3Φ. Wye-Wye connection
# Assumptions:
# 1. Symmetric conditions, except for Lm ~
# 2. R, L, Lm, Rm, and Znetowork should be indicated per phase
# 3. Neutral isolated. Modelled with resistance to ground of 1000 ohms due to convergence issues
#
# ΦA
#                    V_AN_bus
# V_AN --- Znetwork --- / --- R1 ---- L1 -------------------> No Load
#                                            |       |
#                                           Lm ~     Rm
#                                            |       |   
#                                            N       N
...
# REDUCES ORDER OF DIFFERENTIAL EQUATION FROM 2 TO 1
...
def EnergizationWyeWye(V_L, V2_L, L1, L2, Lm, L_A, R1, R2, Rm, phi_0, f, flux_linkage_A_0, flux_linkage_B_0, flux_linkage_C_0, Rnetwork, Lnetwork, K, N, dt, t_stop, change_resolution, bus_energ_voltage_pu, V_source):
    """
    Function to perform the energization simulation of a Wye - Wye transformerm with primary winding isolated
    ...
    Re = 1*10**3 #resistance to ground
    ...
    while t <= t_stop:
        ...
        Il_A = 1/(1+dt/2*R/B_A)*(Il_A_prev + dt/2*((V_AN_prev - I_0_prev*Re)/B_A - Il_A_prev*R/B_A + (V_AN - I_0*Re)/B_A))
        I_A = Il_A + Lm_A_variable/Rm*(Il_A - Il_A_prev)/dt
        flux_linkage_A = flux_linkage_A_prev + Lm_A_variable*(Il_A - Il_A_prev)
        
        #main ΦB 
        B_B = Lm_B_variable*R/Rm + L + Lm_B_variable
        Il_B = 1/(1+dt/2*R/B_B)*(Il_B_prev + dt/2*((V_BN_prev - I_0_prev*Re)/B_B - Il_B_prev*R/B_B + (V_BN - I_0*Re)/B_B))
        I_B = Il_B + Lm_B_variable/Rm*(Il_B - Il_B_prev)/dt
        flux_linkage_B = flux_linkage_B_prev + Lm_B_variable*(Il_B - Il_B_prev)
        
        #main ΦC 
        B_C = Lm_C_variable*R/Rm + L + Lm_C_variable
        Il_C = 1/(1+dt/2*R/B_C)*(Il_C_prev + dt/2*((V_CN_prev - I_0_prev*Re)/B_C - Il_C_prev*R/B_C + (V_CN - I_0*Re)/B_C))
        I_C = Il_C + Lm_C_variable/Rm*(Il_C - Il_C_prev)/dt
        ...
        tolerance = np.array([[tolerance_value], 
                              [tolerance_value], 
                              [tolerance_value]])
        estimate = np.array([[Il_A], [Il_B], [Il_C]])
        solution = np.array([[999999], [999999], [999999]]) #choose any random improbable value
        while np.all(abs(solution - estimate) > tolerance):  
            function = np.array([[Il_A - 1/(1+dt/2*R/B_A)*(Il_A_prev + dt/2*((V_AN_prev - I_0_prev*Re)/B_A - Il_A_prev*R/B_A + (V_AN - I_0*Re)/B_A))], 
                                [Il_B - 1/(1+dt/2*R/B_B)*(Il_B_prev + dt/2*((V_BN_prev - I_0_prev*Re)/B_B - Il_B_prev*R/B_B + (V_BN - I_0*Re)/B_B))], 
                                [Il_C - 1/(1+dt/2*R/B_C)*(Il_C_prev + dt/2*((V_CN_prev - I_0_prev*Re)/B_C - Il_C_prev*R/B_C + (V_CN - I_0*Re)/B_C))]])
            
            #Jacobian matrix
            J = np.array([[1 - 1/(1+dt/2*R/B_A)*dt/2*1/B_A*Re*(1+Lm_A_variable/Rm/dt), - 1/(1+dt/2*R/B_B)*dt/2*1/B_B*Re*(1+Lm_B_variable/Rm/dt), - 1/(1+dt/2*R/B_C)*dt/2*1/B_C*Re*(1+Lm_C_variable/Rm/dt)], 
                          [- 1/(1+dt/2*R/B_A)*dt/2*1/B_A*Re*(1+Lm_A_variable/Rm/dt), 1 - 1/(1+dt/2*R/B_B)*dt/2*1/B_B*Re*(1+Lm_B_variable/Rm/dt), - 1/(1+dt/2*R/B_C)*dt/2*1/B_C*Re*(1+Lm_C_variable/Rm/dt)], 
                          [- 1/(1+dt/2*R/B_A)*dt/2*1/B_A*Re*(1+Lm_A_variable/Rm/dt), - 1/(1+dt/2*R/B_B)*dt/2*1/B_B*Re*(1+Lm_B_variable/Rm/dt), 1 - 1/(1+dt/2*R/B_C)*dt/2*1/B_C*Re*(1+Lm_C_variable/Rm/dt)]])
        ...
        # ΦA bus voltage
        V_AN_bus = V_AN - I_A*Rnetwork - (V_AN-I_A*R-(Vm_A + I_0*Re))/L*Lnetwork
        V_BN_bus = V_BN - I_B*Rnetwork - (V_BN-I_B*R-(Vm_B + I_0*Re))/L*Lnetwork
        # ΦB bus voltage
        V_BN_bus = V_BN - I_B*Rnetwork - (V_BN-I_B*R-(Vm_B + I_0*Re))/L*Lnetwork
        V_CN_bus = V_CN - I_C*Rnetwork - (V_CN-I_C*R-(Vm_C + I_0*Re))/L*Lnetwork
        # ΦC bus voltage
        V_CN_bus = V_CN - I_C*Rnetwork - (V_CN-I_C*R-(Vm_C + I_0*Re))/L*Lnetwork
        V_AN_bus = V_AN - I_A*Rnetwork - (V_AN-I_A*R-(Vm_A + I_0*Re))/L*Lnetwork
    ...
    ax6.plot(freqs_scaling_red, I_A_list_fft_mag_scaling_red, freqs_scaling_red, I_B_list_fft_mag_scaling_red, '--')
    ax6.legend(["I_A freq domain", "I_B freq domain"], loc='right')
    ax6.set_xlabel("Frequency [Hz]")
    ax6.set_ylabel("Magitude [A]")
    ax6.set_xlim([0, 600])
    ax6.set_title("Harmonic content", pad=15)
    ax6.grid()
    try:
        ax6.figure.savefig(r'results/Harmonic_spectrum.png')
    except:
