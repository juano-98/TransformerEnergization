...
# ΦA
#                    V_AN_bus
# V_AN --- Znetwork --- / --- R1 ----L1-------------------> NO CURRENT
#                                          |       |
#                                         Lm ~     Rm
#                                          |       |   
#                                          N       N
...
def EnergizationWyegWye(V_L, V2_L, L1, L2, Lm, L_A, R1, R2, Rm, phi_0, f, flux_linkage_A_0, flux_linkage_B_0, flux_linkage_C_0, Rnetwork, Lnetwork, K, N, dt, t_stop, change_resolution, bus_energ_voltage_pu, V_source):
    """
    Function to perform the energization simulation of a Wye - Wye transformerm with primary winding grounded
    ...
    t = 0 #initial value
    flux_linkage_A = flux_linkage_A_0 #initial conditions
    flux_linkage_B = flux_linkage_B_0 #initial conditions
    flux_linkage_C = flux_linkage_C_0 #initial conditions
    V_AB_bus_rms = V_source #initial value
    V_AN_bus_rms = V_source/math.sqrt(3) #initial value
    V_BC_bus_rms = V_source #initial value
    V_BN_bus_rms = V_source/math.sqrt(3) #initial value
    V_CA_bus_rms = V_source #initial value
    V_CN_bus_rms = V_source/math.sqrt(3) #initial value
    ...
    while t <= t_stop:
        ...
        Il_A = 1/(1+dt/2*R/B_A)*(Il_A_prev + dt/2*((V_AN_prev)/B_A - Il_A_prev*R/B_A + (V_AN)/B_A))
        ...
        Il_B = 1/(1+dt/2*R/B_B)*(Il_B_prev + dt/2*((V_BN_prev)/B_B - Il_B_prev*R/B_B + (V_BN)/B_B))
        ...
        Il_C = 1/(1+dt/2*R/B_C)*(Il_C_prev + dt/2*((V_CN_prev)/B_C - Il_C_prev*R/B_C + (V_CN)/B_C))
    ...
    #n_cycles = 5 #number of cycles to represent
    ...
    ax1.legend(["I_A", "I_B", "I_C"], loc='right')
...
if __name__ == "__main__":
    ...
    f = 50 #Hz
    w = 2*math.pi*f #rad/s
    flux_linkage_M = V_L/math.sqrt(3)/((2*math.pi/math.sqrt(2))*f)
    flux_linkage_A_0 = 0#pu
    flux_linkage_B_0 = 0 #pu
    flux_linkage_C_0 = 0 #pu
    flux_linkage_A_0 = flux_linkage_A_0*flux_linkage_M #Wb
    flux_linkage_B_0 = flux_linkage_B_0*flux_linkage_M #Wb
    flux_linkage_C_0 = flux_linkage_C_0*flux_linkage_M #Wb
    Rnetwork = 0 #ohm
    Lnetwork = 0.022 #H 
