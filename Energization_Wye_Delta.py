...
# 3Φ. Wye-Delta connection
# Assumptions:
# 1. Symmetric conditions, except for Lm ~
# 2. R, L, Lm, Rm, and Znetowork should be indicated per phase
# 3. Neutral isolated. Modelled forcing sum of currents to be zero = infinite impedance to ground
#
# ΦA
#                    V_AN_bus
# V_AN --- Znetwork --- / --- R ----L-------------------> I_delta
#                                       |       |
#                                      Lm ~     Rm
#                                       |       |   
#                                       N       N
...
def EnergizationWyeDelta(V_L, V2_L, L1, L2, Lm, L_A, R1, R2, Rm, phi_0, f, flux_linkage_A_0, flux_linkage_B_0, flux_linkage_C_0, Rnetwork, Lnetwork, K, N, dt, t_stop, change_resolution, bus_energ_voltage_pu, V_source):
    """
    Function to perform the energization simulation of a Wye - Delta transformerm with primary winding isolated
    ...
    while t <= t_stop:
        ...
        #main ΦC 
        Il_C = - Il_A - Il_B - I_delta*3 - Lm_A_variable/Rm*(Il_A - Il_A_prev)/dt - Lm_B_variable/Rm*(Il_B - Il_B_prev)/dt - Lm_C_variable/Rm*(Il_C - Il_C_prev)/dt
        I_C = Il_C + Lm_C_variable/Rm*(Il_C - Il_C_prev)/dt + I_delta
        flux_linkage_C = flux_linkage_C_prev + Lm_C_variable*(Il_C - Il_C_prev)
        
        alisado = 9 #before it had a purpose, now it does not affect the final result
        ...
        I_delta_prev_prev = I_delta_prev
        I_delta_prev = I_delta
        I_delta = (((flux_linkage_sum - flux_linkage_sum_prev - 3*(R21*dt/2-L21)*I_delta_prev)/(3*(R21*dt/2+L21))) + (alisado-1)*I_delta_prev)/alisado #se aplica un alisado porque el cálculo de I_delta es exagerado
        #---- FIRST ESTIMATES ----
        ...
        while np.all(abs(solution - estimate) > tolerance):  
            ...
            J = np.array([[1, 0, 0, -1/(1+dt/2*R/B_A)*dt/2*(1/B_A*(-R-L/dt))], 
                          ...
                          [1 + Lm_A_variable/Rm/dt, 1 + Lm_B_variable/Rm/dt, 1 + Lm_C_variable/Rm/dt, 3], 
